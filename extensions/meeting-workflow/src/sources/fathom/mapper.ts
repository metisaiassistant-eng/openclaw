...
export type FathomAccountContext = {
  accountKey: string;
  label: string;
  email?: string;
};
...
export function mapFathomPayloadToMeetingEvent(
  payload: unknown,
  account: FathomAccountContext,
): MeetingEventV1 {
  ...
  return {
    ...
    endedAt,
    transcript,
    participants: ensureParticipantFallback(parseParticipants(obj.participants), title),
