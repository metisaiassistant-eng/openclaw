...
def parse_task_date(value: str, field: str) -> int:
    try:
        return int(datetime.strptime(value, "%Y-%m-%d").timestamp() * 1000)
    except ValueError as exc:
        raise ValidationError(f"Invalid {field} format. Use YYYY-MM-DD") from exc


def sanitize_tag(value: str) -> str:
    return sanitize_text(" ".join(value.split()), "tag", MAX_TAG_LENGTH)


def collect_tags(values: list[str] | None) -> list[str]:
    if not values:
        return []
...
def cmd_create_task(args: argparse.Namespace) -> None:
    ...
    if args.start:
        body["start_date"] = parse_task_date(args.start, "start date")
    if args.due:
        body["due_date"] = parse_task_date(args.due, "due date")
    if args.assignee:
        body["assignees"] = [validate_identifier(args.assignee, "assignee")]
    if args.tag:
        body["tags"] = collect_tags(args.tag)
