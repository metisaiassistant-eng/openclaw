...
description: Maintain this fork's canonical `my-main` branch and its clean Linux deployment checkout. Use when syncing from upstream, creating feature branches, logging into the VPS, deploying to the server, or cleaning server-local state out of the repo checkout.
